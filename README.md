...
- **WindowEc** — limit retries based on failure count.
- **Window** — restrict per-node usage per interval.
- **Random** — randomly choose entries.
- **Simple** — round-robin sequential allocator.
- **Time** — allocate resources on timed intervals.
...
async fn main() {
    ...
    let lb = WindowLoadBalancer::new(vec![(2, "node 1"), (2, "node 2")]);
    ...
    let lb = WindowLoadBalancer::new_interval(
